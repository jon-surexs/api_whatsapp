// src/controllers/contactAdvisorController.js
...
// desde el flujo contact_advisor.
//
// Los datos del producto se almacenan en:
//
// payload.contact_advisor.description
//
...
const listContactAdvisor = async (req, res) => {
  ...
  try {
    ...
    // ------------------------------------------------------------
    // Filtramos solo leads del producto contact_advisor
    // ------------------------------------------------------------

    const leads = await OutboxLead.find({
      "payload.product": "CONTACT_ADVISOR"
    })
    .sort({ createdAt: -1 })
    .lean();
    ...
    const rows = leads.map(lead => {
      ...
      // ------------------------------------------------------------
      // Este campo proviene del flujo contact_advisor
      // y contiene el texto que el usuario escribió
      // describiendo el seguro que necesita.
      // ------------------------------------------------------------

      const description = lead.payload?.contact_advisor?.description || "";
